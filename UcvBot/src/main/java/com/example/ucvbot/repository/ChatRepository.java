...
public interface ChatRepository extends JpaRepository<Chat, Long> {
        ...
        // @Query("SELECT new com.example.ucvbot.dto.ChatSummaryDTO(c.id, c.title,
        // c.userUID) " +
        // "FROM Chat c WHERE c.userUID = :userUID")
        // List<ChatSummaryDTO> findChatSummariesByUserUID(@Param("userUID") String
        // userUID);
        //
        // @Query(value = "SELECT user_name FROM chat GROUP BY user_name, useruid ORDER
        // BY COUNT(*) DESC LIMIT 1", nativeQuery = true)
        ...
        //
        //
        // @Query(value = "SELECT c.user_name, COUNT(m.statement) AS
        // cantidad_statements\n" +
        // "FROM chat c\n" +
        ...
        //
        //
        // @Query(value = "SELECT MAX(unix_time) - MIN(unix_time) as duration FROM
        // message GROUP BY chat_id ORDER BY duration DESC LIMIT 1;\n",nativeQuery =
        // true)
        // String longerTalkTime();

        @Query("SELECT new com.example.ucvbot.dto.ChatSummaryDTO(c.v_id, c.v_title, c.v_student.v_userUID) " +
                        "FROM Chat c WHERE c.v_student.v_userUID = :userUID")
